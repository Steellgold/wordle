import { LetterStatus, Line } from "@/lib/types/wordle.type";
...
export const isValidWord = (letterArray: Line[], targetWord: string, lineIndexToCheck: number): ValidPosition[] => {
  ...
  const word = letterArray[lineIndexToCheck].map((letter) => letter.value.toLowerCase()).join("");
  const wordDecomposed = word.split("");
  const targetWordDecomposed = targetWord.split("");

  let positions: ValidPosition[] = wordDecomposed.map((value) => ({ value, status: "unknown" }));
  ...
  letterArray.forEach((line) => {
    line.forEach((entry) => {
      if (entry.status === 'well-placed') {
        lettersUsed.add(entry.value.toLowerCase());
      }
    });
  });
...
export const containsLetter = (letterArray: Line[], letter: string): boolean => {
  return letterArray.some((line) => line.some((data) => data.value === letter));
}
